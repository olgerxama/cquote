...
export default function OwnerLeadsPage() {
  ...
  const { data: leads = [], isLoading } = useQuery({
    queryKey: ['owner-leads-full'],
    queryFn: async () => {
      const { data } = await supabase
        .from('leads')
        .select('id, firm_id, full_name, email, service_type, status, property_value, created_at, instruction_submitted_at')
  ...
  const filtered = useMemo(() => {
    ...
    return leads.filter((lead) => {
      ...
      if (!q) return true
      return [lead.full_name, lead.email, lead.service_type, firmNameById[lead.firm_id] || '']
        .some((v) => String(v).toLowerCase().includes(q))
    })
  }, [leads, search, status, firmNameById])

  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground">Platform Leads</h1>
        <p className="text-muted-foreground mt-1">Cross-firm lead feed with status, service, and instruction visibility.</p>
      </div>
      ...
      <div className="mb-4 flex flex-col md:flex-row gap-3">
        <input
          ...
          placeholder="Search lead, email, service, or firm..."
