...
async function buildQuotePdfAttachment(p: {
  ...
  filename: string
}): Promise<{ filename: string; content: string; encoding: 'base64' }> {
  ...
  y -= 30
  if (p.referenceCode) draw(`Ref: ${p.referenceCode}`, rightAlignedX(`Ref: ${p.referenceCode}`, 10), y, 10, false, muted)

  y -= 34
  page.drawLine({ start: { x: left, y }, end: { x: right, y }, thickness: 1, color: rgb(0.9, 0.92, 0.95) })
  y -= 20
  draw('Prepared for', left, y, 10, true, muted)
  draw(p.leadName, left, y - 16, 12)
  if (p.serviceType) {
  ...
  if (p.totals.discountTotal > 0) rows.push({ label: 'Discount', value: `−${formatCurrency(p.totals.discountTotal)}`, color: green })
  rows.push({ label: 'VAT', value: formatCurrency(p.totals.vatAmount) })
  rows.push({ label: 'Grand Total', value: formatCurrency(p.totals.grandTotal), bold: true })
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    let generatedQuoteItems: QuotePdfItem[] = []
    ...
    if (lead.status !== 'review' && totals) {
      ...
      if (!quoteError && quote) {
        ...
        if (quoteItems?.length) {
          ...
          generatedQuoteItems = items.map((item) => ({
    ...
    if (firm.auto_send_quote_emails && lead.status !== 'review' && totals) {
      try {
        ...
        const result = await sendEmail({
          ...
          attachments: [
            await buildQuotePdfAttachment({
              firmName: firm.name,
              leadName: lead.full_name,
              serviceType: lead.service_type,
              referenceCode: referenceCode || leadId,
              documentType: 'estimate',
              items: generatedQuoteItems,
              totals: {
                subtotal: Number(totals.subtotal || 0),
                discountTotal: 0,
                vatAmount: Number(totals.vatTotal || 0),
                grandTotal: Number(totals.grandTotal || 0),
              },
              filename: `estimate-${referenceCode || leadId}.pdf`,
            }),
          ],
        })
